window.APP = {
  ...
  mounted() {
    post('http://cnc_chat/loaded', JSON.stringify({}));
    this.listener = window.addEventListener('message', (event) => {
      ...
      if (this[item.type]) {
        ...
        if (item.isTeam) {
            if (item.team == 1){
                document.getElementById("prefix").style.color = "orange"
  ...
  methods: {
    ...
    send(e) {
      if(this.message !== '') {
        post('http://cnc_chat/chatResult', JSON.stringify({
    ...
    hideInput(canceled = false) {
      if (canceled) {
        post('http://cnc_chat/chatResult', JSON.stringify({ canceled }));
      }
      this.message = '';
